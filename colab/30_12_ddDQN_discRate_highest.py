...
with open(os.path.join(save_path, f"log-{now_str}.txt"), 'w') as log:
    message_lr = f"lr_optimizer = {lr_optimizer}\n"
    print(message_lr, end='')
    log.write(message_lr)
    for episode in range(n_episodes):
        ...
        #message = "(Episode {: 5d}/{})  Gold {: 4d}  undiscounted_return {: 6d}  steps {: 4d}  eps {:.3f}  lr {:3.2f}  ({})\n".format(episode+1, n_episodes, env.state.score, undiscounted_return, step + 1, epsilon, lr_optimizer, constants.agent_state_id2str[env.state.status])
