...
class MinerEnv:
    ...
    def get_state(self):
        ...
        for i, bot in enumerate(self.socket.bots):
            channel_bot = np.zeros_like(channel0)
            x_bot = bot.info.posx
            y_bot = bot.info.posy
            x_bot_out_of_map = x_bot < 0 or x_bot >= constants.width
            y_bot_out_of_map = y_bot < 0 or y_bot >= constants.height
            if x_bot_out_of_map or y_bot_out_of_map:
                pass
            else:
                channel_bot[y_bot, x_bot] = -3  # just a random number to represent bots/enemies
            channels234.append(channel_bot)

        state = np.stack((channel0, channel1, channels234[0], channels234[1], channels234[2]), axis=-1)
        return state
