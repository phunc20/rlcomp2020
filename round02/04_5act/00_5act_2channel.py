...
def play_one_step(env, state, epsilon):
    ...
    try:
        #logging.debug(f"pos=({env.state.x:2d},{env.state.y:2d}), terrain={next_state[...,0][env.state.y, env.state.x]:5.1f}, lastAction={constants02.action_id2str[action]:>4}, energy={env.state.energy:2d}, energy_pre={env.state.energy_pre:2d}, score={env.state.score}, reward={reward:.1f}")
        ## Seems like we don't need `score`
        logging.debug(f"pos=({env.state.x:2d},{env.state.y:2d}), terrain={next_state[...,0][env.state.y, env.state.x]:5.1f}, lastAction={constants02.action_id2str[action]:>5}, energy={env.state.energy:2d}, energy_pre={env.state.energy_pre:2d}, reward={reward:6.1f}")
    except IndexError:
        ...
        logging.debug(f"pos=({env.state.x:2d},{env.state.y:2d}), lastAction={constants02.action_id2str[action]:>5}, energy={env.state.energy:2d}, energy_pre={env.state.energy_pre:2d}, reward={reward:6.1f}")
    #next_state, reward, done, info = env.step(action)
