...
import datetime 
import numpy as np
from constants import maps, gold_total
...
INPUTNUM = 27 #The number of input values for the DQN model
ACTIONNUM = 4  #The number of actions output from the DQN model
MAP_MAX_X = 21 #Width of the Map
MAP_MAX_Y = 9  #Height of the Map

DQNAgent = DQN(INPUTNUM, ACTIONNUM, epsilon=0.01)
...
for episode_i in range(0, N_EPISODE):
    try:
        ...
        pretty = minerEnv.get_pretty_state()
        remaining_gold = pretty[pretty>0].sum()
        #for g in minerEnv.socket.stepState.golds:
        #    remaining_gold += g["amount"]
        total_gold = gold_total(np.array(maps[mapID]))
        print("(Episode {: 5d})   cumulated_reward: {:7.1f}   Steps: {: 4d}   eps: {:5.2f}   remaining_gold: {: 7d}/{: 7d}  ({})".format(episode_i+1, total_reward, step + 1, DQNAgent.epsilon, remaining_gold, total_gold, game_over_reason[minerEnv.state.status]))
        ...
        history[episode_i] = total_reward

np.save("reward_history", history)
