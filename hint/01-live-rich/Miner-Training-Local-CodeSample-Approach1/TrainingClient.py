...
DQNAgent = DQN(INPUTNUM, ACTIONNUM, epsilon=0.7, epsilon_decay = 0.9999,)
h5 = "TrainedModels/DQNmodel_20200819-0443_ep19950.h5"
        ...
        try:
            ...
            if (np.mod(episode_i + 1, SAVE_NETWORK) == 0 and train == True):
                ...
                DQNAgent.save_model("TrainedModels/",
                                    "DQNmodel_" + now.strftime("%Y%m%d-%H%M") + "_ep" + str(2*N_EPISODE + episode_i + 1))
            ...
            message = "(Episode {: 5d})   Gold: {: 4d}  cumulated_reward: {:7.1f}   Steps: {: 3d}   eps: {:5.2f}  ({})\n".format(episode_i+1, minerEnv.state.score, total_reward, step + 1, DQNAgent.epsilon, game_over_reason[minerEnv.state.status])
            print(message, end="")
            log.write(message)
